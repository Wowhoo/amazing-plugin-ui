...
(function(window, document, $, Color, undefined) {
    ...
    ColorInput.prototype = {
        ...
        bindEvent: function() {
            ...
            this.$picker.on('click.colorInput', function() {
                return false;
            });
            this.$wrap.on('click.colorInput', function() {
                return false;
            });
        },
...
(function($) {
    $.colorInput.registerComponent('gradient', {
        ...
        init: function(api) {
            ...
            api.$element.on('colorInput::close', function() {
                if (api.isGradient) {
                    self.setGradient();
                }
                return false;
            ...
            this.$degree.on('blur.colorInput', function() {
                ...
                self.setDegree(deg);
                return false;
            }).on('keydown.colorInput', function(e) {
                var key = e.keyCode || e.which;
                if (key === 13) {
                    $(this).blur();
                    return false;
